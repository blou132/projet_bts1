...
class JoueurController extends BaseController
{
    ...
    public function store(): void
    {
        ...
        if ($errors) {
            $_SESSION['pilotes_errors'] = $errors;
            $params = [];
            if ($idEcurie > 0) {
                $params['focus'] = $idEcurie;
            }
            $this->redirectTo('ecuries', $params);
        }
        $pdo->prepare('INSERT INTO joueurs(nom,prenom,poste,id_equipe,photo) VALUES (?,?,?,?,?)')
            ->execute([$nom,$prenom,$poste,$idEcurie,$photo]);
        $_SESSION['pilotes_flash'] = 'Pilote ajoute.';
        $this->redirectTo('ecuries', ['focus' => $idEcurie]);
    }
    ...
    public function update(): void
    {
        ...
        $_SESSION['pilotes_flash'] = 'Pilote mis a jour.';
        $this->redirectTo('ecuries', ['focus' => $idEcurie]);
    }
    ...
    public function delete(): void
    {
        ...
        if ($id <= 0) {
            ...
            $stmt = $pdo->prepare('SELECT id_equipe FROM joueurs WHERE id=?');
            $stmt->execute([$id]);
            $focusId = (int)$stmt->fetchColumn();
        ...
        $_SESSION['pilotes_flash'] = 'Pilote supprime.';
        $params = [];
        if ($focusId > 0) {
            $params['focus'] = $focusId;
        }

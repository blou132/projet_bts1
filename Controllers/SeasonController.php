...
{
    private const MAX_RESULT_POSITION = 22;
    private const POINTS_BY_POSITION = [
        1 => 25,
        2 => 18,
        3 => 15,
        4 => 12,
    ...
    public function course(): void
    {
        ...
        $this->render('course.lame.php', [
            ...
            'maxResultPosition' => self::MAX_RESULT_POSITION,
            'betWindow' => $betWindow,
            'userBet' => $userBet,
    ...
    public function addResult(): void
    {
        ...
        $points = $this->pointsForPosition($position);

        $errors = $this->validateResult($courseId, $piloteId, $position);
        if ($errors) {
    ...
    public function updateResult(): void
    {
        ...
        $errors = $this->validateResult($courseId, $piloteId, $position, $resultId);
        if ($errors) {
            if ($this->isAjaxRequest()) {
                $course = $this->fetchCourse($courseId);
                if ($course) {
    ...
    private function pointsForPosition(int $position): int
    {
        return self::POINTS_BY_POSITION[$position] ?? 0;
    }
     ...
     */
    private function validateResult(int $courseId, int $piloteId, int $position, ?int $excludeId = null): array
    {
        $errors = [];
        ...
        } elseif ($position > self::MAX_RESULT_POSITION) {
            $errors[] = 'La position maximale est ' . self::MAX_RESULT_POSITION . '.';
        }
